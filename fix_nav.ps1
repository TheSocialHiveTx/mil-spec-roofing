$files = Get-ChildItem -Path services -Filter index.html -Recurse | Where-Object { $_.Directory.Name -ne "services" }
...
$pattern2 = '(?is)(<h4 class="font-bold text-slate-900 uppercase tracking-wider mb-4 border-b pb-2">All Services</h4>\s*<ul class="space-y-2 text-sm">)\s*<li><a href="[^"]+" class="block py-2 text-slate-600 hover:text-blue-600\s*hover:pl-2 transition-all border-b border-slate-100">[^<]+</a></li>'
$replacement2 = '$1
...
Write-Output "Done fixing service pages."
